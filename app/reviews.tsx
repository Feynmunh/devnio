...
export function ThreeDScrollTriggerContainer({
  ...
  const velocityFactor = useTransform(smoothVelocity, (v: number) => {
    const sign = v < 0 ? -1 : 1;
...
function ThreeDScrollTriggerRowImpl({
  ...
  useAnimationFrame((time: number) => {
    ...
    const xTransform = useTransform(x, (v: number) => `translate3d(${-v}px,0,0)`);
...
function ThreeDScrollTriggerRowLocal(props: ThreeDScrollTriggerRowProps) {
  ...
  const localVelocityFactor = useTransform(localSmoothVelocity, (v: number) => {
    const sign = v < 0 ? -1 : 1;
    const magnitude = Math.min(5, (Math.abs(v) / 1000) * 5);
    return sign * magnitude;
  });
